...
dgp_backdoor_frontdoor_iv_b1_violated_alt <- function(n, beta = 3) {
  ...
  # Treatment (a function of U, there is an unblocked backdoor path from A and Y, (B1) violated)
  pA.UC <- expit(C[,1] + expit(C[,2]) + sin(C[,3]) + 5*U)
  A1 <- rbinom(n, size = 1, prob = pA.UC)
  A0 <- rbinom(n, size = 1, prob = 1-pA.UC)
...
dgp_backdoor_frontdoor_iv_iv_correct_b2f1f2_violated <- function(n, beta = 3) {
  ...
  pM1.AUC <- expit(5 * 1 - 1 + C[,2] + U) # pM1.AUC and pM0.AUC are the same for never takers and always takers, exclusion restriction satisfied
