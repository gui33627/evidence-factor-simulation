...
# Confounders: age, sex, BMI, past history of heart disease (prevchd)
...
data <- df %>% select(diabetes_cont_2, smoke_bin_1, hyperten_bin_2, prevhyp_bin_1, 
                      age_cont_1, sex_bin_1, bmi_cont_1, prevchd_bin_1) %>% 
  rename(Y = diabetes_cont_2, A = smoke_bin_1, M = hyperten_bin_2, Z = prevhyp_bin_1,
         X1 = age_cont_1, X2 = sex_bin_1, X3 = bmi_cont_1, X4 = prevchd_bin_1)

n <- nrow(data)

# estimate using AIPW (backdoor IF)
pi.model <- gam(A ~ s(X1) + X2 + s(X3) + X4, family = 'binomial', data = data)
mu.model <- gam(Y ~ A + s(X1) + X2 + s(X3) + X4, family = 'gaussian', data = data)
...
A.model <- gam(A ~ s(X1) + X2 + s(X3) + X4, family = 'binomial', data = data)
M.model <- gam(M ~ A + s(X1) + X2 + s(X3) + X4, family = 'binomial', data = data)
Y.model <- gam(Y ~ A + M + s(X1) + X2 + s(X3) + X4, family = 'gaussian', data = data)
