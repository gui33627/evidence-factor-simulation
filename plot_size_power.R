...
library(tidyverse)
prefix <- "/Users/junhui/Library/CloudStorage/GoogleDrive-junhuiyang@umass.edu/My Drive/UMASS/simulations.hpc/results/"
...
df_fdoor_iv <- as.data.frame(rbind(df_fdoor_iv_250, df_fdoor_iv_500, df_fdoor_iv_750, df_fdoor_iv_1000)) %>% 
  ...
  filter(!(f1 == T& f2 ==T& f3 == T& i1 ==T& i2 == F& i3 == T& i4 == T & hypothesis == "N"))
...
df_backdoor_frontdoor_null_all <- df_backdoor_frontdoor %>% filter(hypothesis == "N") %>% 
  ...
  ggplot(., aes(x = sample_size, y = size, color = nonzero_functionals, linetype = nonzero_functionals)) + 
  geom_line() + 
...
df_backdoor_frontdoor_alt_all <- df_backdoor_frontdoor %>% filter(hypothesis == "A") %>% 
  mutate(nonzero_functionals = ifelse(frontdoor_true_functional == 1 & backdoor_true_functional == 1, "Identified functional in wrong model \u2260 0", "Identified functional in wrong model = 0")) %>% 
  ggplot(., aes(x = sample_size, y = power, color = nonzero_functionals, linetype = nonzero_functionals)) + 
  geom_line() +
  ggh4x::facet_nested("Frontdoor assumptions" + frontdoor ~ "Backdoor assumptions" + backdoor) +
...
df_backdoor_frontdoor_iv_null_all <- df_backdoor_frontdoor_iv %>% filter(hypothesis == "N") %>% 
  mutate(nonzero_functionals = ifelse((backdoor_true_functional == 1 & frontdoor_true_functional == 1) | (backdoor_true_functional == 1 & iv_true_functional == 1) | (frontdoor_true_functional == 1 & iv_true_functional == 1), "Identified functional in wrong model \u2260 0", "Identified functional in wrong model = 0")) %>% 
